...
Секреты хранятся в `/opt/gorec/*.env` (`bot.env`, `stack.env`) с режимом `600`. Рядом создаётся symlink `/opt/gorec/.env` → `stack.env`, чтобы bare `docker compose` из project dir подставлял `BOT_ENV`/`CONFIG_ROOT`/`DATA_ROOT`. Manager не печатает содержимое этих файлов в обычных командах; Telegram Bot Token передаётся `curl` через stdin-конфиг и не появляется в аргументах процесса.
